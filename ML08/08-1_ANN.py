...
# It is important to scale the data before using neural networks
...
#   - transform: perform tha actual standardization by centering and scaling
# You can do each step separately (as we did before) or use fit_transform() to combine both steps:
X_train = scaler.fit_transform(X_train)

# On the test set, we only apply the transformation:
# We use the mean and standard deviation that we learned from the test set:
...
# Our diabetes dataset has many more negative (no diabetes) than positive (diabetes) cases:
...
#   - Since the Accuracy measures the perfomance in both classes equally, it is not very informative
...
#   - Precision: How many of the predicted positives are actually positive (applies to 2-class problems such as diabetes).
